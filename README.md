...
Consul for Claude Code is an orchestration toolkit with pre-built skills, worker agents, and deployment configs that turns a product description into deployed, tested software — autonomously.
...
Consul handles decomposition, coding, testing, visual design, and deployment across backend, frontend, and infrastructure — with you in the loop only when it matters.
...
```
  You describe it           Consul builds it                  You push to deploy
...
1. **Onboard** — Run the setup wizard, pointed at a project directory (existing or not). Four questions in Quick Start, the full stack interview in Custom. Consul creates the repo, installs skills, agents, hooks, and the brief board, pushes it to GitHub, and provisions the Render database and services from `render.yaml`.
...
```bash
python3 ~/consul/onboard.py --set-render-key     # → ~/.claude/settings.json env block
```

That is the only Render credential Consul uses — CLI, API calls, hooks, every project. `render login` is never required (its browser token expires; it remains a fallback).

### Step 1–3: clone → `/start` (or the wizard directly) → new repo

```bash
cd ~/consul && claude        # then type /start — Claude drives everything below
# or, developers, run the wizard yourself:
python3 ~/consul/onboard.py ~/code/my-new-app [--quick|--custom]
# or fully non-interactive:
python3 ~/consul/onboard.py ~/code/my-new-app --quick --yes --name "My App" --description "..." --domain "..." --login no
```
...
# Consul takes over: infra → backend → frontend → deploy, turn after turn
/status
