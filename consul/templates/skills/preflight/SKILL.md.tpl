...
The script prints one line per check: `[PASS]`, `[WARN]` (Consul can work around it or it only matters later), or `[FAIL]` (the run will block on it) with a `→ fix` line. Exit code 1 means at least one FAIL.
