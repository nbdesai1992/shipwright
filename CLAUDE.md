...
Every change to Consul is measured against those three steps. Nothing may become a per-project manual prerequisite between them — no Dashboard clicking, no file editing, no "first go create X". The only manual setup allowed is account-level and one-time (Render payment method, Render↔GitHub connection, Render API key, optional Clerk app), and it is documented in GETTING-STARTED.md. If a new feature needs something from the human, it is asked for in the wizard or reported by `preflight.py` with a fix line; if a script can do it, a script does it (`provision.py` creates Render resources — the Blueprint Instance is optional).
...
1. Clone this repo
2. From your target project directory: `python /path/to/consul/onboard.py`
...
- Generic skills should contain NO project-specific content (stack-level references to Render, render.yaml, and Clerk are accepted in V1 — Consul is Render-only)
- Templates use `{{PLACEHOLDER}}` syntax (simple string replacement, no Jinja2)
- Render is the only deploy platform. The onboarding wizard offers `render` or `none`; a new adapter needs `consul/templates/skills/deploy/{platform}/SKILL.md.tpl` AND a wizard choice in `onboard.py`
...
- Earlier product names stay readable: `onboard.py` still loads `shipwright.json` / `factory-config.json`, and the orchestrate skill still treats push policy `shipwright` / `factory` as `consul`. Don't remove these; projects onboarded before the renames depend on them. Never ship an executable named `consul` (it would shadow HashiCorp's CLI)
